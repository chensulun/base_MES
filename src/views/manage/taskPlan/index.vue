  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="计划状态" prop="taskPlanStatus">
        <el-select
          v-model="queryParams.taskPlanStatus"
          placeholder="请选择计划状态"
          clearable
          size="small"
        >
    ...
    >
      ...
      <el-table-column
        label="项目名称"
        align="center"
        prop="taskPlanProjectName"
      />
      <el-table-column
        label="生产类型"
        align="center"
        prop="taskPlanProductionType"
      />
      <el-table-column
        label="总计划量"
        align="center"
        prop="taskPlanQuantity"
      />
      <el-table-column
        label="生产计划状态"
        align="center"
        prop="taskPlanStatus"
        :formatter="taskPlanStatusFormat"
      />
      <el-table-column
        label="创建时间"
        align="center"
    ...
    >
      <el-button type="primary" plain icon="el-icon-plus" @click="selectAdd"
        >新增任务
      </el-button>
    ...
    >
      ...
      <el-table
        v-loading="loading"
        :data="taskList"
        @selection-change="taskHandleSelectionChange"
      >
        <el-table-column type="selection" width="55" align="center" />
        <el-table-column
          label="任务编号"
        ...
        >
          <template slot-scope="scope">
            <span>{{
              parseTime(scope.row.taskSupplyTime, "{y}-{m}-{d}")
            }}</span>
          </template>
        </el-table-column>
        <el-table-column label="地址" align="center" prop="taskAddress" />
        ...
        <el-form ref="form" :model="form" :rules="rules" label-width="120px">
          ...
          <el-col :span="12">
            <el-form-item label="生产类型" prop="taskProductionType">
              <el-select
                v-model="form.taskProductionType"
                @change="projectSignDataSelectChange"
                placeholder="请选择生产类型"
                style="width: 100%"
              >
                <el-option
          ...
          <el-col :span="12">
            <el-form-item label="计划量" prop="taskPlannedQuantity">
              <el-input
                v-model="form.taskPlannedQuantity"
                type="number"
                placeholder="请输入计划量"
              />
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="发车间隔时间" prop="taskDepartureInterval">
              <el-input
                v-model="form.taskDepartureInterval"
                placeholder="请输入发车间隔时间"
              />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      this.form.taskIds = Array.from(
        new Set(this.addTaskList.map((item) => item.taskId))
      ).join(",");
      this.form.taskPlanProjectIds = Array.from(
        new Set(this.addTaskList.map((item) => item.projectId))
      ).join(",");
      this.form.taskPlanProjectName = Array.from(
        new Set(this.addTaskList.map((item) => item.projectName))
      ).join(",");
      this.form.taskPlanProductionType = Array.from(
        new Set(this.addTaskList.map((item) => item.taskProductionType))
      ).join(",");
