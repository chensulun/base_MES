        ...
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="站点权限">
              <el-select style="width: 100%" v-model="form.msIdList" @change="forceUpdate" multiple placeholder="请选择">
                <el-option
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getDept(row.deptId).then(response => {
        ...
        if (this.form.msIds) {
          this.form.msIdList = this.form.msIds.split(",").map(id => Number(id));
        }
        this.open = true;
        this.title = "修改部门";
      });
    ...
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.form.msIds = this.form.msIdList.join(",");
